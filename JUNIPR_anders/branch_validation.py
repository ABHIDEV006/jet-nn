...
parser.add_argument('-m', '--model_path', default='../../data/junipr/d_0.03_charge')
...
[daughters, endings, mothers, (discrete_p_splittings, discrete_q_splittings), mother_momenta] = load_data(input_path, 
        n_events=n_events, batch_size=batch_size, split_p_q=True,
        p_granularity=p_granularity, q_granularity=q_granularity)

x = [[*a] for a in zip(daughters, mother_momenta, [m[1] for m in mothers], [p[0] for p in discrete_p_splittings])]

model = JUNIPR_class.JUNIPR_branching(model_path=model_path,
    p_granularity=p_granularity, q_granularity=q_granularity)
