...
for j in [['u_jets.out', ucharges, upt, Upt], ['d_jets.out', dcharges, dpt, Dpt]]:
...
for i, k in enumerate(k_array):
    ...
    plt.ylabel('dN/dQ')
    plt.legend()
    plt.savefig('more_figures/jetcharge_k{}.png'.format(k))
...
plt.legend()
plt.savefig('more_figures/roc.png')
plt.figure(30)
plt.title('sic')
plt.rc('text', usetex=True)
plt.xlabel(r'$\epsilon_s$, Down Quark Jet Identification')
...
plt.legend()
plt.savefig('more_figures/sic.png')
