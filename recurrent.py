...
if len(sys.argv) != 2:
  print('Must supply only timesteps dimension length')
  sys.exit(1)
...
batch = 6000
saves = sorted(glob.glob('../data/length_{}_batch_{}_rnn/weights*.hdf5'.format(length, batch)))
model = None
if len(saves) != 0:
    print('Loading saved model...')
    model = load_model(saves[-1])
    n_epochs -= int(saves[-1][-8:-5])
...
input_vecs = np.load('../data/length_{}_rnn_processed_vec.npy'.format(length))
print('Data loaded...')
...
if not os.path.exists('../data/length_{}_batch_{}_rnn/'.format(length, batch)):
  os.makedirs('../data/length_{}_batch_{}_rnn/'.format(length, batch))
checkpointer = Checkpoint('../data/length_{}_batch_{}_rnn/weights'.format(length, batch) + '{epoch:03d}.hdf5', TOTAL_EPOCHS - n_epochs)
print('Fitting...')
...
plt.plot(tpr, 1-fpr)
plt.title('roc')
plt.savefig('/n/home03/tculp/jets/figures/rnn_roc_length_{}_batch_{}.png'.format(length, batch))
plt.figure(2)
plt.plot(tpr, tpr/np.sqrt(fpr))
plt.title('sic')
plt.savefig('/n/home03/tculp/jets/figures/rnn_sic_length_{}_batch_{}.png'.format(length, batch))
