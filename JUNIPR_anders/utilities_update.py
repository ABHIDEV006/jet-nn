...
def load_data(data_path, n_events, batch_size, granularity, save_dir='./saved_data', reload=False):
    data_basename = os.path.splitext(os.path.basename(data_path))[0]
    save_path = save_dir + '/'+ data_basename + '_N' + str(n_events) + '_BS' + str(batch_size) +'_G' + str(granularity) + '.pickled'
    if not reload and os.path.exists(save_path):
        ...
        discrete_splittings = [get_discrete_splittings(s, granularity)  for s in splittings_batches]
...
def splitting_to_daughters(splitting, parent):
    ...
    z, theta, phi, delta = splitting
    p_E, p_th, p_phi = parent
    ...
    return np.asarray([*d1_abs, *d2_abs])
...
def get_discrete_splittings(splittings, granularity):  
    '''Returns a one_hot representation of the discreteized splitting and a corresponding mask for padded values'''
    discrete_splittings = [np.append([splitting_to_i(cont_splitting, granularity) for cont_splitting in splittings[i]], 0) for i in range(len(splittings))] 
    discrete_splittings = tf.keras.preprocessing.sequence.pad_sequences(discrete_splittings, dtype='int32', padding='post', value=0)
...
def get_mother_momenta(tree, parent_id):
    ...
    mother_momenta = [np.append(np.asarray([shift_mom(tree[i][id]) for id in parent]),[[-1]*3], axis=0) for i, parent in enumerate(parent_id)]
