...
if len(sys.argv) != 2:
  print('Must include only length of timesteps dimension')
  sys.exit(1)
...
isd = []
dists = []
for j in ['../data/d_jets.out', '../data/u_jets.out']:
    with open(j, 'r') as f:
        charge = []
        pt = []
        eta = []
...
np.save('../data/length_{}_rnn_processed_vec.npy'.format(timesteps), ret)
