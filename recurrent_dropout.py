...
length = 120
batch = 3000
saves = sorted(glob.glob('../data/dropout_length_{}_batch_{}_rnn/weights*.hdf5'.format(length, batch)))
model = None
if len(saves) != 0:
    ...
    model.add(GRU(100, return_sequences=True, dropout=0.3))
    model.add(GRU(90, return_sequences=True, dropout=0.3))
    model.add(GRU(80, return_sequences=True, dropout=0.3))
    model.add(GRU(70, return_sequences=True, dropout=0.3))
    model.add(GRU(60, return_sequences=True, dropout=0.3))
    model.add(GRU(50, return_sequences=True, dropout=0.3))
    model.add(GRU(40, return_sequences=True, dropout=0.3))
    model.add(GRU(30, return_sequences=True, dropout=0.3))
    model.add(GRU(20, return_sequences=True, dropout=0.3))
    model.add(GRU(10, return_sequences=True, dropout=0.3))
    model.add(GRU(5, dropout=0.3))
    model.add(Dense(64, activation='relu'))
    model.add(Dense(2, activation='softmax'))
    model.compile(loss='categorical_crossentropy', optimizer=SGD(lr=0.1, momentum=0.95), metrics=['accuracy'])
    #model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    print('Model compiled...')
    
print(model.summary())

input_vecs = np.load('../data/rnn_processed_vec.npy')
print('Data loaded...')
...
checkpointer = Checkpoint('../data/dropout_length_{}_batch_{}_rnn/weights'.format(length, batch) + '{epoch:03d}.hdf5', TOTAL_EPOCHS - n_epochs)
print('Fitting...')
...
plt.plot(tpr, 1-fpr)
plt.title('roc')
plt.savefig('/n/home03/tculp/jets/figures/dropout_rnn_roc_length_{}_batch_{}.png'.format(length, batch))
plt.figure(2)
plt.plot(tpr, tpr/np.sqrt(fpr))
plt.title('sic')
plt.savefig('/n/home03/tculp/jets/figures/dropout_rnn_sic_length_{}_batch_{}.png'.format(length, batch))
